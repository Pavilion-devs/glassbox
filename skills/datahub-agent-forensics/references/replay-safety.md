...
Receipt signer authority uses the same rule: a key embedded in a DBOM is not trusted
unless its ID and public-key fingerprint satisfy the operator registry. `RETIRED`
preserves previously admitted history; `REVOKED` does not.

## History and comparison

Execution must produce a new signed DBOM linked by the prior payload digest. Failed
replay also gets a receipt. Never overwrite the source.
